...
#2. 將網址更新: Line的後台Webhook URL
#3. 啟動應用程式，點擊vscode有上的播放鍵
...
def handle_message(event):
    with ApiClient(configuration) as api_client:
        ...
        print("User Profile的顯示名稱: ", user_profile.display_name)
        print("使用者傳入的文字訊息是:", user_msg)
        print("#" * 30)
        bot_msg = TextMessage(text=f"{user_profile.display_name}，你剛才說的是： {user_msg}")
        if user_msg in faq:
            ...
            completion = client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "system", "content": """你是一個很棒的AI，什麼事情都難不倒你，如果使用者不知道如何操作，記得要提醒使用者"""},
                    {
                        "role": "user",
                        "content": user_msg
                    }
